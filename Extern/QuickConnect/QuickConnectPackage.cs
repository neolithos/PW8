...
namespace Neo.PerfectWorking.QuickConnect
{
	public sealed class QuickConnectPackage : PwPackageBase, IPwIdleAction, IDisposable
	{
		...
		private sealed class Connection
		{
			...
			public Connection(IPwGlobal global, PwAction action, string remotePath, string localPath, string credentialTarget)
			{
				this.global = global;
		...
		private void RefreshConnectionState()
		{
			...
			try
			{
				...
				{
					var nr = (NETRESOURCE)Marshal.PtrToStructure(new IntPtr(dataPtr.ToInt32() + (i * Marshal.SizeOf(typeof(NETRESOURCE)))), typeof(NETRESOURCE));
		...
		{
			imageConnected = new BitmapImage(new Uri("pack://application:,,,/PW.QuickConnect;component/Resources/NetDriveConnected.png", UriKind.Absolute));
			imageDisconnected = new BitmapImage(new Uri("pack://application:,,,/PW.QuickConnect;component/Resources/NetDriveDisconnected.png", UriKind.Absolute));
		} // sctor
